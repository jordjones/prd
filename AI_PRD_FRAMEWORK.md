# AI-Optimized PRD Framework

> **Purpose:** This document defines the requirements for writing Product Requirements Documents that AI coding agents (Claude Code, OpenAI Codex, Cursor, etc.) can reliably execute. It is itself structured as a CLAUDE.md / AGENTS.md-consumable reference.
>
> **Audience:** Engineers, PMs, and founders who use AI agents to build software.
>
> **Core Principle:** The specification IS the source code. AI agents cannot infer intent from omission. Everything explicit gets built. Everything omitted gets invented — or ignored.

---

## SECTION 1: THE EIGHT REQUIREMENTS FOR REQUIREMENTS

Each requirement below is numbered, imperative, and includes a justification rooted in the research report that produced this framework.

### R1: DELETE BEFORE YOU SPECIFY

Every requirement in the PRD SHALL have a named human owner. Before drafting, challenge every proposed feature: "Does this need to exist?" If you are not adding back at least 10% of deleted requirements during review, you have not deleted enough.

**Justification:** Musk's 5-step algorithm places "question requirements" and "delete" before "simplify" or "optimize." AI agents will faithfully implement unnecessary features, wasting tokens, time, and introducing surface area for bugs. CodeRabbit data shows AI generates 1.7x more bugs than humans — every unnecessary feature multiplies that risk.

### R2: USE STRUCTURED MARKDOWN WITH HEADING HIERARCHY

The PRD SHALL be written in Markdown (.md) with a clear heading hierarchy (H1 > H2 > H3). Requirements SHALL use numbered lists or EARS-format keywords (SHALL, SHALL NOT, WHEN, IF). Code examples SHALL use fenced code blocks with language identifiers.

**Justification:** A Vanderbilt/William & Mary study found GPT-4o achieves near-perfect accuracy on structured content but drops to 20–40% on narrative prose. Every major AI coding tool natively parses Markdown. Imperative instructions ("Use TypeScript strict mode") achieve 94% compliance vs 73% for descriptive statements ("The project uses TypeScript").

### R3: STATE NON-GOALS EXPLICITLY

The PRD SHALL include a "Non-Goals / Out of Scope" section listing capabilities the system MUST NOT implement. This section SHALL be at least 20% as long as the goals section.

**Justification:** AI agents cannot infer boundaries from omission. If a PRD for an API doesn't say "no authentication," the agent may add OAuth, JWT, session management, and a user table — because most APIs need auth. Galileo's failure-mode taxonomy documents agents that deleted vendor records because the prompt said "remove outdated entries" without defining "outdated." Explicit exclusions are the only reliable scope control.

### R4: PHASE THE WORK INTO 5–6 STAGES WITH 30–50 REQUIREMENTS EACH

The PRD SHALL decompose the product into sequential phases. Each phase SHALL be independently buildable and testable. Each phase SHALL contain no more than 50 discrete requirements.

**Justification:** Frontier LLMs reliably follow approximately 150–200 instructions before performance degrades. CLAUDE.md files under 200 lines achieve 92%+ compliance, dropping to 71% beyond 400 lines. Phasing also enables checkpoint verification — catching drift before it compounds across the entire codebase. Single-task prompts reduce error rates by 65% on projects over 10,000 lines.

### R5: SEPARATE WHAT / HOW / STEPS INTO DISTINCT SECTIONS

The PRD SHALL separate requirements (WHAT the system does), design (HOW it does it), and tasks (STEPS to build it) into clearly labeled sections or files.

**Justification:** David Lapsley's three-document pattern (requirements.md / design.md / tasks.md) is now implemented by Amazon Kiro, GitHub Spec Kit, and dozens of practitioner workflows. Mixing "what" with "how" causes agents to conflate constraints with implementation choices, producing rigid code that satisfies the letter of the spec while violating its intent.

### R6: INCLUDE EXECUTABLE VERIFICATION COMMANDS

Every phase and every major feature SHALL include at least one executable command or test that verifies correct implementation. The PRD SHALL specify exact build, test, and lint commands with expected outputs.

**Justification:** Anthropic's official best-practice workflow is TDD: write tests first, confirm they fail, then ask the agent to make them pass without modifying the tests. IEEE Spectrum documented that newer LLMs produce code that "fails to perform as intended but on the surface seems to run successfully" — including generating fake data to match expected output rather than surfacing errors. Without executable verification, you cannot distinguish working code from plausible code.

### R7: DEFINE ERROR STATES, EDGE CASES, AND EMPTY STATES

Every data-displaying component SHALL specify its empty state, loading state, and error state. Every input SHALL specify validation rules with exact criteria. Every API endpoint SHALL specify its failure responses.

**Justification:** The most common AI coding failure is the "happy path only" implementation. CodeRabbit found security issues at 1.5–2x human rates, with excessive I/O at 8x. Silent failures — where the agent handles errors by suppressing them or generating plausible-looking fake output — are the most dangerous class of AI bug. Explicit error specifications are the primary defense.

### R8: FRONT-LOAD CRITICAL CONTEXT AND BUDGET TOKENS

The most important requirements SHALL appear in the first 20% of the document. The total PRD for any single agent session SHALL not exceed 4,000 words (~5,300 tokens). Supplementary detail SHALL be referenced via file paths, not inlined.

**Justification:** The "lost in the middle" effect means LLMs attend more to content at the beginning and end of context. Claude Code's effective working context is ~140K tokens after system overhead and compaction buffers. Every word of specification competes with file contents and tool results. Progressive disclosure — referencing detail files rather than inlining everything — lets the agent load context on demand.

---

## SECTION 2: PRD TEMPLATE SKELETON

Use this skeleton as a starting point. Delete sections that don't apply (R1).
...
## SECTION 5: QUICK REFERENCE — THE CHECKLIST

Before handing any PRD to an AI coding agent, verify:

- [ ] Every requirement uses SHALL / SHALL NOT / WHEN / IF (not "should" or "could")
- [ ] Tech stack includes exact versions and at least one negative constraint
- [ ] Non-goals section exists and has 5+ explicit exclusions
- [ ] Every endpoint/component specifies its success response AND its error response
- [ ] At least one copy-pasteable verification command exists per phase
- [ ] Total word count is under 4,000 for a single-session spec
- [ ] The most critical requirements appear in the first 20% of the document
- [ ] Boundaries section defines ALWAYS / ASK FIRST / NEVER rules
- [ ] No requirement uses "appropriate," "proper," "good," "clean," or "best practices" without defining what that means in concrete terms
- [ ] A human who has never seen this project could read the PRD and build the same system the author imagined — because if a human can't, an AI certainly can't
